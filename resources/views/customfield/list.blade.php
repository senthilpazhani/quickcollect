...
<div id="customfieldModal" class="modal fade modal-fade-in-scale-up" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="post" id="customfield_form">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <select id="datatype" name="datatype" data-plugin="select2" class="form-control">
                    ...
                    <div class="form-group">
                        <input type="checkbox" class="icheckbox-primary" id="required" name="required"
...
<style>.form-group{margin-bottom:0.5rem;}</style>
    ...
    <link rel="stylesheet" href="{{URL::asset('/global/vendor/icheck/icheck.css') }}">
@endsection
@section('jsscript')
    ...
    $(document).ready(function(){
        ...
        $(document).ready(function(){
            var table = $('#listitem_table').DataTable({
                ...
                "serverSide":true,
                "autoWidth": false,
                "ajax":"{{ route('customfield.getdata') }}",
                "columns":[
                    {"data":"id"},
                    {"data":"name","width": "45%"},
                    {"data":"datatype","width": "25%"},
                    {"data":"required","width": "20%"},
            ...
            $('#add_data').click(function(){
                ...
                $('#customfield_form')[0].reset();
                $('#form_output').html('');
                $('#button_action').val('insert');
                $('.modal-title').text('Add Custom Field');
                $('#action').html('Add');
            });
            $('#customfield_form').on('submit',function(event){
                event.preventDefault();
                var form_data = $(this).serialize();

                $.ajax({
                    url:"{{ route('customfield.postdata') }}",
                    method:"POST",
                    data:form_data,
                    dataType:"json",
            ...
            $(document).on('click','.edit',function(){
                ...
                $.ajax({
                    url:"{{ route('customfield.fetchdata') }}",
                    method:"get",
                    data:{id:id},
                    dataType:'json',
                    success:function(data){
                       ...
                       // $('#required').val(data.required);
                        if (data.required){alert(data.required);
                            //$('#required').attr('checked','checked');
                            $('#required').iCheck('check');
                        } else {
                            $('#required').iCheck('uncheck'); }
                        $('#customfield_id').val(id);
                        $('#customfieldModal').modal('show');
                        $('#form_output').html('');
                        $('#action').html('Edit');
                        $('.modal-title').text('Edit Custom Field');
            ...
            $(document).on('click','.delete',function(){
                if(confirm("Are you sure to delete this Custom Field?"))
                {
                    var id=$(this).attr("data_id");
                    $.ajax({
                        url:"{{ route('customfield.deletedata') }}",
                        method:"get",
                        data:{id:id},
                        dataType:'json',
                        success:function(data){
...
@section('pagescript_bot')
   <!-- Page -->
        ...
        <script src="{{URL::asset('global/vendor/icheck/icheck.min.js') }}"></script>
        <script src="{{URL::asset('global/js/Plugin/icheck.js') }}"></script>
