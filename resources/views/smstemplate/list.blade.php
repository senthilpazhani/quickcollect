...
<div id="smstemplateModal" class="modal fade modal-fade-in-scale-up w-full" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="post" id="smstemplate_form">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="template" id="template" class="form-control"  rows="3"></textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    <input type="hidden" name="smstemplate_id" id="smstemplate_id" value="" />
    ...
    $(document).ready(function(){
        ...
        $(document).ready(function(){
            var table = $('#listitem_table').DataTable({
                ...
                "serverSide":true,
                "autoWidth": false,
                "ajax":"{{ route('smstemplate.getdata') }}",
                "columns":[
                    {"data":"id"},
                    {"data":"name","width": "30%"},
                    {"data":"template","width": "60%"},
            ...
            $('#smstemplate_form').on('submit',function(event){
                event.preventDefault();
                var form_data = $(this).serialize();

                $.ajax({
                    url:"{{ route('smstemplate.postdata') }}",
                    method:"POST",
                    data:form_data,
                    dataType:"json",
            ...
            $(document).on('click','.edit',function(){
                ...
                $.ajax({
                    url:"{{ route('smstemplate.fetchdata') }}",
                    method:"get",
                    data:{id:id},
                    dataType:'json',
                    success:function(data){
            ...
            $(document).on('click','.delete',function(){
                if(confirm("Are you sure to delete this Template?"))
                {
                    var id=$(this).attr("data_id");
                    $.ajax({
                        url:"{{ route('smstemplate.deletedata') }}",
                        method:"get",
                        data:{id:id},
                        dataType:'json',
                        success:function(data){
